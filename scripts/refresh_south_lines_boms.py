"""
refresh_south_lines_boms.py — extracts real itemized BOM + RM cost for the
1,715 size-variant codes across the 10 South MRP lines (Sanibel, Ardene,
SPK CT, Organica, Zenimo, Crystal, Grand Palais, Vivah, Double Decker,
Italiano — scripts/south_lines_variants.json, built by
build_south_lines_variants.py) across all 4 months, same two-pass
streaming approach as refresh_bom_data.py / refresh_comfort_supreme_boms.py,
saving into the same bom_store/costing_store databases the tool reads from.
Additive only — the existing tracked SKUs are untouched.
"""
import json
import os
import sys
...
def main():
    ...
    print(f"Extracting BOMs for {len(item_codes)} South-line size-variant codes across {len(MONTHS)} months...")
    matched_by_code = {}
